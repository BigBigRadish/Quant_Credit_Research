%This program implements a neighborhood choice monte carlo simulation used for spatial equilibrium problems
...
M = 3;
%Number of school/neighborhood amenities that affect neighborhood desirability
K = 3;
%P will measure the number of school characteristics that don't affect choice.
P = 1;
...
%Simulating preference parameters Delta.  Need one delta for each characteristic/amenity combination
Delta = randn(L+M,K);
Delta_O = Delta(1:L,:);
...
%To see efficiency gains from vectorization, compare the times for the above loops when L = 500, M = 500.  Gains from vectorization tend to appear when the loops they replace are long.
...
%Reshaping the matrix of choice-specific utilities into an (N*S)x1 vector. The first N elements represent each student's willingness to pay for
...
%Creating a matrix of constraints where each row constraints each student to choose exactly one school.
...
%Have not constrained each element of Assignment to be 0 or 1, only to be between 0 and 1. But minimizing value will only be between 0 and 1 if
%there are multiple allocations which yield the same total utility (in which case any arbitrary linear combination between these two allocations
%will also yield maximum surplus.  Will choose an allocation among the optimal set by assigning all values greater than .5 to 1, less than .5 to 0
...
%Rearranging the assignment vector into a matrix where each student has a one in a single column (the one corresponding to the school they have chosen.
Assignment2 = reshape(Assignment2,N,S);

Assignment3 = Assignment2';
%Want to convert logical matrix into a single vector with the index number of the chosen school for each student.

Assignment3 = logical(Assignment3);
Temp = repmat((1:S)',1,N);
Schoolassign = Temp(Assignment3);

%Suppose you want to find the willingness to pay of each individual at their chosen school. Could try using: 
WTP_choice = WTP(:,Schoolassign);

%But you would end up with an NxN matrix where kth column consists of all students WTP at the school chosen by the kth student.  

%To select individual, non-contiguous points in a matrix, need to use linear indexing.

%Find the linear index values of the elements of the WTP matrix corresponding to students' chosen schools.
Lin_index =  sub2ind(size(WTP),(1:N)',Schoolassign);

%Evaluate the WTP matrix at the linear indexed list to obtain a vector of each student's WTP at their chosen school.
...
%Or can get school means for all S schools for all L characteristics in one line. But is it faster?
tic
...
%Or you could loop over S instead of L or M.  Not helpful in this case because S > L, S > M.
tic
X_Os_3 = zeros(S,L);
X_Us_3 = zeros(S,M);
