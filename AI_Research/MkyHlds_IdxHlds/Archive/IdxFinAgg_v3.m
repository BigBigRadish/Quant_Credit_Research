function [ AnaAgg ] = IdxFinAgg_v3( ds, col )
...
for i = 1:size(AnaAgg,1)
    ...
    weight_info = sum(tmp.Mkt_Pct_Weight(~I,:));

    if not(isempty(tmp))
        tmp(1,3:end) = mat2dataset(sum(dn(tmp(:,2))*ones(1,size(tmp,2)-2).*dn(tmp(:,3:end)),1)/weight_info);
        tmp(2:end,:)=[];
        tmp{1,2} = weight;
        %tmp.dataWeight(1) = weight_noinfo;
        
        AnaAgg(i,1:end-1) = tmp;
        AnaAgg{i,end} = weight_info;
    end
